...
📊 进度：M1 距 deadline X 天，104 任务完成 N / 104 (P%)，平均每天需 X.X 个
🚧 阻塞：[列还在的 + 标老化 > 3 天的]
...
**示例**：
```
📊 进度：M1 距 deadline 16 天，104 任务完成 0 / 104 (0%)，平均每天需 6.5 个（9 通道并行）
...
## 3.5 编码原则（编码前必读 · Karpathy 派生）

> 来源：[Andrej Karpathy 关于 LLM 编码通病的观察](https://x.com/karpathy/status/2015883857489522876) + [multica-ai/andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills)。
> hmrouter 是 9 agent 并行改 4 万行 Go + 建 enterprise 模块，LLM 编码 4 个通病在并行场景被**放大**：
> ① 替你做错误假设不核对就跑 ② 过度复杂化（1000 行做 100 行的事）③ 改/删它没理解的代码 ④ 弱成功标准要反复澄清。
> 下面 4 条是降事故率的**硬约束**，所有 Backend/Frontend/DB/DevOps agent 编码前必读。
> **权衡**：偏"谨慎 > 速度"，琐碎任务用判断力别教条。
...
- 不加没要求的功能；单次使用的代码不做抽象。
- 没要求的"灵活性 / 可配置性"不要做；不为不可能发生的场景写错误处理。
- 写了 200 行而 50 行能搞定 → 重写。自问："senior 工程师会不会嫌这过度复杂？" 会 → 简化。
...
改已有代码时：
- 不"改进"邻近代码、注释、格式；不重构没坏的东西；匹配现有风格，即使你会用别的写法。
- 发现无关的死代码 → 提一句，**别删**。

你的改动造成孤儿时：
- 删掉**你的改动**导致没用的 import / 变量 / 函数；不删**预先存在**的死代码（除非被要求）。
...
- 文件所有权见 [docs/agent-collaboration.md](docs/agent-collaboration.md)，不碰其他 agent owner 的文件。
...
多步任务先报一个简短计划（每步带验证检查点）。强成功标准让你能独立 loop；弱标准（"让它能用"）会逼你反复澄清。
...
- [ ] 没动其他 agent owner 的文件（见 docs/agent-collaboration.md）
...
> **原则在起作用的标志**：diff 里没有不必要的改动 / 因过度复杂返工次数下降 / 澄清问题出现在实现前而非犯错后。
